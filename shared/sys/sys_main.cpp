...
void *Sys_LoadSPGameDll( const char *name, GetGameAPIProc **GetGameAPI )
{
	...
	if (!libHandle) {
		...
		libHandle = Sys_LoadDllFromPaths( filename, gamedir, searchPaths, numPaths,
											SEARCH_PATH_MOD | SEARCH_PATH_OPENJK | SEARCH_PATH_ROOT,
											__FUNCTION__ );
		if ( !libHandle )
			return NULL;
	}
...
#endif
	{
		...
		if ( !libHandle )
		{
			...
			if (!libHandle) {
				...
				libHandle = Sys_LoadDllFromPaths( filename, gamedir, searchPaths, numPaths, SEARCH_PATH_OPENJK | SEARCH_PATH_MOD, __FUNCTION__ );
				if ( !libHandle )
					return NULL;
			}
		}
